...
def load_config():
    ...
    # オプション設定（Secretsが未設定だと空文字列になるため、空の場合はデフォルト値を使う）
    def _int_env(key, default):
        val = os.environ.get(key, "").strip()
        return int(val) if val else default

    config["INSTANCE_NAME"]  = os.environ.get("INSTANCE_NAME", "").strip() or "ampere-a1-free"
    config["OCPU_COUNT"]     = _int_env("OCPU_COUNT",     4)
    config["MEMORY_GB"]      = _int_env("MEMORY_GB",      24)
    config["BOOT_VOLUME_GB"] = _int_env("BOOT_VOLUME_GB", 50)
    config["SSH_PUBLIC_KEY"] = os.environ.get("SSH_PUBLIC_KEY", "").strip()
...
# ワークフローがPEMを書き出したファイルパス
OCI_KEY_FILE = "/tmp/oci_api_key.pem"


def build_oci_config(cfg):
    """OCI SDK用のconfigとComputeClientを構築する"""
    # 秘密鍵はワークフロー側で /tmp/oci_api_key.pem に書き出し済み
    # key_file 方式を使うことでCR+LF・改行リテラル等の問題を回避する
    if not os.path.exists(OCI_KEY_FILE):
        logger.error(
            f"秘密鍵ファイルが見つかりません: {OCI_KEY_FILE}\n"
            "ワークフローの 'PEM鍵ファイルを書き出す' ステップが正常に完了しているか確認してください。"
        )
        sys.exit(1)
